...
- **Hugging Face model checkpoint:** [`YaphetL/gpt2-therapy-generator`](https://huggingface.co/YaphetL/gpt2-therapy-generator/tree/main) — use this as the starting point for testing inference against the NLP model. Note it's a GPT-2/GPT-Neo **text-generation** checkpoint, not a text-classification one, so it returns generated text rather than `{label, score}` pairs; swapping it in as `HF_MODEL_ID` as-is would need `backend/app.py`'s response parsing (currently written for classifier output) adjusted to handle a generation response before it'd work end-to-end.
